...
public class ProgramaGenetico {
			...
			int ai_numIter,double ad_prob_cruce,double ad_prob_mut,double elitismo_in,int mutacion)
	{
		ProfMaxima		= profMax;
		tamanoPob		= tamPob;
		pob				= new Cromosoma[tamanoPob];
		pobIntermedia	= new Cromosoma[tamanoPob];
    	...
    	this.num_pob_elite      = (int) (this.tamanoPob * this.elitismo);
	...
	public void seleccionTorneo(){
		...
		for(int i=0; i< tamanoPob - this.getNum_pob_elite(); i++)	
		{
	...
	public  void seleccionRanking()
	{
		...
		for(int i=0; i< tamanoPob - this.getNum_pob_elite() ; i++)
		{
			prob = aleatorio(); 
			posSeleccionado = 0;
			while((posSeleccionado < tamanoPob- 1)&&(prob > selec[posSeleccionado]))
			{
				posSeleccionado++;
			}
	...
	public  void seleccionEstocasticoUniversal(){
		...
		{
			prob = aleatorio();
			prob = (prob+i-1)/(tamanoPob - this.getNum_pob_elite());
			posSeleccionado = 0;
			while((posSeleccionado < tamanoPob -1)&&(prob > pob[posSeleccionado].getPuntuacion_neta_acumulada()))
			{
				posSeleccionado++;
			}
			seleccion[i]= posSeleccionado;
		...
		private double[] rankPopulation(){
			double Beta_ = 1.5;
			double[] fitnessSegments = new double[this.tamanoPob];
			for(int i=0 ; i<fitnessSegments.length ; i++){
				double probOfIth = (double)i/this.tamanoPob;
				probOfIth = probOfIth*2*(Beta_-1);
				probOfIth = Beta_ - probOfIth;
				probOfIth = (double)probOfIth*((double)1/this.tamanoPob);
				if(i!=0)
		...
		private void ordena_peores() {
			...
			for(int i=0 ; i< tamanoPob; i++){
				//this.pobIntermedia[i + tamanoPob - this.getNum_pob_elite()]= pob[indices_seleccionados.get(i)];
		...
		private void selecciona_peores() {
			...
			for(int i=0 ; i< this.getNum_pob_elite() ; i++){
				pobIntermedia[i + tamanoPob - this.getNum_pob_elite()].copiaCromosoma(pob[indices_seleccionados.get(i)])  ;
			}		
		}
		...
		{
			int[] seleccionCruce= new int[tamanoPob - this.getNum_pob_elite()];
			int numSelecCruce=0;
			double prob;
		...
		public  void evaluarPoblacion()
		{
			...
			setMedioAptitud(sumAptitud/ tamanoPob);
			setMinimoAptitud(aptitudPeor);
			setMaximoAptitudNeta(aptitudMejorNeta);
			setMedioAptitudNeta(sumAptitudNeta/ tamanoPob);
